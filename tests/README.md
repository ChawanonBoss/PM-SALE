...
| `close_job_test.py` | ซื้อขาย-only "ปิดงาน" button: disabled with a tooltip until the job has a saved photo, never appears at all on a โครงการ row, its modal's date defaults to today but is backdatable, requires the signed document before it will confirm, the saved file is its own `pm_files` row (`role:'closing'`) excluded from the general "ไฟล์แนบ" list/count, closing flips the ซื้อขาย list's own สถานะ badge to "ปิดงานแล้ว" and disables both it and "ลบ" for good, and disabled row-action buttons are actually styled as disabled (grey, not-allowed cursor) |
| `projects_sample_data_test.py` | โครงการ's admin-only "+ เพิ่มข้อมูลตัวอย่าง" button creates 5 projects with every field filled in, a fully-done 5-step Action Plan, installments one short of the total ("งวดงาน" reads ready for the last one), and real pm_photos rows for both sets (not just the photoCounts) - makes its own throwaway sample company only when none already exists, refuses to duplicate itself, and "ลบข้อมูลตัวอย่างทั้งหมด" sweeps the projects, that customer/company, and the photo rows together with no orphans left behind |
